...
We focus on **pre-authorised flow only**. A credential is only issued to a party that is already signed in through an existing authentication/authorization system (e.g. ACM/IDM or any other authentication/authorization mechanism used by a partner). From then on, this user can use their credential to log in to our system or systems that trust our issuer.
...
2. The user attempts to access the distribution through its `accessURL`.
   ...
   * 2.3 Once the (mock) payment is complete, the payment provider module modifies the user's session so that they have access to the distribution. This session modification also counts as the pre-authorisation needed to start the OID4VCI credential flow in step 3.
3. The user's session now has the required access privileges to start a pre-authorised OID4VCI flow with the issuer service of the dataset provider. In the demonstration, the DECIDe issuer component is reused with default account and role information added to the credential.
