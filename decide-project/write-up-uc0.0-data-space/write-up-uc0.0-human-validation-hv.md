...
| ------------------------------------------- | -------------------------- | ----------------- | ---------- | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| Human validation feedback (`oa:Annotation`) | Data space authentication  | Belgium ([Ghent](https://catalog.decide.lblod.info/dcat/datasets/79f9ed1c-94cb-5a77-a17e-af9a192acfcb)) / Germany ([Bamberg](https://catalog.decide.lblod.info/dcat/datasets/e177b2ce-a73f-5530-83d9-940b03a8e8fc), [Freiburg](https://catalog.decide.lblod.info/dcat/datasets/3ba1b22f-bb96-509b-a061-3a4e837042f2)) | Government | Yes                       | Yes - shown as aggregate number in Human Validation Tool, Codelist mapping votes are reused by the codelist mapping service for training. |
...
To complement the manual review carried out through the HVT interface, two SPARQL queries were configured in the "export_csv" script ([documentation](https://github.com/lblod/app-decide/tree/development/scripts/project/export_csv)) to export the current state of AI annotations together with their human review counts. The script runs both queries against the triplestore and writes the results to two CSV files. [The first query](https://github.com/lblod/app-decide/blob/development/scripts/project/export_csv/queries/codelist-labeling-validations.sparql) lists every expression–codelist label pair, along with contextual properties such as the decision name, decision content and the AI agent responsible for the match, plus the number of thumbs-up and thumbs-down reviews it received in the HVT. [The second query](https://github.com/lblod/app-decide/blob/development/scripts/project/export_csv/queries/entity-recognition-validations.sparql) lists every expression–entity pair, along with properties such as the decision name, decision content, subject, found predicate, found entity, entity type and AI agent, again together with its thumbs-up and thumbs-down counts. This made it possible to review and sanity-check large batches of enrichments and their human acceptance at once, without needing to page through the HVT interface or write ad-hoc SPARQL.
