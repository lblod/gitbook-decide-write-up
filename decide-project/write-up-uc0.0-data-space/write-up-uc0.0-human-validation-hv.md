...
To complement the manual review carried out through the HVT interface, two SPARQL queries ([TODO: link to codelist query], [TODO: link to entity query]) and a script ([TODO: link to script]) were created to export the current state of AI annotations together with their human review counts. The script runs both queries against the triplestore and writes the results to four CSV files. The first query lists every expression–codelist label pair, along with contextual properties such as the decision name, decision content and the AI agent responsible for the match, plus the number of thumbs-up and thumbs-down reviews it received in the HVT. The second query lists every expression–entity pair, along with properties such as the decision name, decision content, subject, found predicate, found entity, entity type and AI agent, again together with its thumbs-up and thumbs-down counts. This made it possible to review and sanity-check large batches of enrichments and their human acceptance at once, without needing to page through the HVT interface or write ad-hoc SPARQL.
