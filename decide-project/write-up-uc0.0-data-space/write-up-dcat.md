...
We should make the different DCAT feeds of Ghent reference to each other. Ghent has a DCAT feed on the open data platform and is working on a DCAT feed on the linked open data server. From a user perspective it would be logical if these would link to each other.
