...
UC2 depends on the data infrastructure established in UC0.0. The Expressions that users can ask about in UC2 are ingested, standardized, and stored as linked data through the UC0.0 pipelines. The Virtuoso SPARQL endpoint that UC2 queries for document metadata is part of the [semantic.works](https://semantic.works/) stack set up in UC0.0. Without the ingestion pipelines and the triplestore, UC2 has no corpus to search over.
